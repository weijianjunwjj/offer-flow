...
async function runDev(): Promise<void> {
  ...
  try {
    ...
    console.log('[B3 临时联调] 仅使用系统临时 schema v2 SQLite；退出会清理全部临时数据。');
    console.log(`[B3 临时联调] API: ${session.apiUrl}`);
    console.log(`[B3 临时联调] Web: ${session.webUrl}/#/profile-versions`);
    console.log(`[B3 临时联调] Temp DB: ${session.dbPath}`);
...
async function main(): Promise<void> {
  if (process.argv.includes('--smoke')) {
    ...
    console.log('[B3 临时联调 smoke] PASS', JSON.stringify(report));
    return;
  }
  await runDev();
}
