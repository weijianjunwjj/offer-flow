...
describe('buildRuntimeFlags — 两开关', () => {
  ...
  it('不再注入 OFFERFLOW_WAKE_SCHEDULER（wake task 归提权 CLI 管理，backend 不 mutation）', () => {
    expect(buildRuntimeFlags()).not.toHaveProperty('OFFERFLOW_WAKE_SCHEDULER');
  });

  it('不发明 API key / secret：仅两个 capability flag，值全为 true', () => {
