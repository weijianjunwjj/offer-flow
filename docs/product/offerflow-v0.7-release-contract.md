...
- 当前 App 版本：`0.6.2`
- 当前发布结论：**v0.7 产品实施中；可信求职记忆底座已完成；G1 全局岗位匹配画像 MVP 已经用户验收（2026-07-15）；动态画像正式版（G4）与阶段策略（G5）尚未完成；禁止发布**
...
| 当前状态 | **G3 工程主体已实现（schema v4、history-import/funnel 领域与 server、前端页面与路由，自动化测试全绿），产品收口中，尚未用户验收**。用户实际验收发现历史补录无法完整走通、基础漏斗默认分组无效，正在补齐隔离验收环境与漏斗信息架构。 |
| 阻塞项 | 禁止用 B 的 legacy backfill 冒充用户历史补录产品；测试全绿、工程实现完成不等同于产品验收通过；G3 必须待用户在隔离验收环境中人工走通历史补录全流程、且基础漏斗总览/分组/岗位族/可信度符合要求后才能标记“已验收”。 |
