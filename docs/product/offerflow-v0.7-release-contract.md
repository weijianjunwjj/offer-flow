...
| 当前状态 | **G3 产品逻辑已在隔离 sandbox 中经用户验收；真实数据库已受控升级到 schema v4，正式环境已开放历史补录，并完成不写正式业务数据的最小生产烟测，均通过；G3 仍未正式验收**。Snapshot v4 尚未发布，最终签收待用户对生产环境做最后确认。 |
| 阻塞项 | 禁止用 B 的 legacy backfill 冒充用户历史补录产品；测试全绿、工程实现完成、生产烟测通过均不等同于最终产品验收；G3 最终签收、Snapshot 发布、合并 main、进入 G4 均待用户明确裁决。 |
