...
| §6.5、§11 | 两层历史补录 | 历史基线 + 详细事件草稿 | 历史补录向导 | `server/history-import` | 最小补录、详细补录、回忆限制 | 补录预览与确认截图 | G3 | sandbox 产品验收通过；真实库已升级至 v4、正式环境已开放、最小生产烟测通过；G3 仍未正式验收 |
| §19.3、§27“历史与统计” | 基础漏斗与分组反馈 | Funnel aggregate | 基础漏斗/反馈概览 | `server/funnel` | 城市/岗位族/渠道/简历分组；零样本 | 漏斗与排除说明截图 | G3 | sandbox 产品验收通过；真实库已升级至 v4、正式环境已开放、最小生产烟测通过；G3 仍未正式验收 |
...
G1、G2 验收只将各自覆盖的矩阵条目标记为“已实现，已用户验收”；R3–R5 及 2.1/2.3/2.5/2.6/2.7 的正式产品结果仍未完成并继续阻塞发布。
