...
| §6.5、§11 | 两层历史补录 | 历史基线 + 详细事件草稿 | 历史补录向导 | `server/history-import` | 最小补录、详细补录、回忆限制 | 补录预览与确认截图 | G3 | 工程主体已实现，产品收口中，尚未用户验收 |
| §19.3、§27“历史与统计” | 基础漏斗与分组反馈 | Funnel aggregate | 基础漏斗/反馈概览 | `server/funnel` | 城市/岗位族/渠道/简历分组；零样本 | 漏斗与排除说明截图 | G3 | 工程主体已实现，产品收口中，尚未用户验收 |
