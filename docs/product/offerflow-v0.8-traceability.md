...
| RC-11 | RadarPromotion | P0-11 / US-09 | 4.12 / 13.4 | 8 | V8-6 | Partial — **岗位晋升功能人工验收通过（2026-07-27）**，RC-11 **未** Done。已完成并有证据：预览零写入（直读四表计数）、确认写入 Job/Application/FeedbackEvent/Promotion 各一条且 UI 展示 ID 与库内逐字段一致、幂等重放（200 + `created=false` + 零新增）、`no_response` 双端 409 且零残留、深度钳制、原子失败整体回滚、端到端剧本（`promotion:e2e` 11/11 连跑两次）；vitest 1353、vue-tsc、build 全绿。**缺口：** 反向追踪无证据；「撤销雷达动作不删正式事实」依赖未实现的 RC-10，无法验证；link 模式暂无 UI 入口（仅 API 验证）。生产入口 DISABLED、schema 仍 v7、生产迁移未授权 | 晋升预览、幂等、反向追踪；阶段性验收证据 `docs/evidence/offerflow-v0.8-v8-6-promotion-partial-acceptance-2026-07-27.md` |
| RC-12 | 可靠任务与发布闭环 | P0-12 / US-10 / 12.2 | 4.8 / 10 | 6 / 9 | V8-4/6 | Partial — V8-4 单岗位可靠分析任务已实现并人工验收通过（2026-07-27）（确定性 createTask、固定 Snapshot、状态机、最多一次结构修复、原子写入、cancel/迟到结果抑制、进程恢复、retry 复用快照、current/stale 投影，自动化全绿）；**发布闭环/迁移/恢复演练（V8-6）未开始** | 故障日志、migration、恢复、截图；实施证据 `docs/technical/offerflow-v0.8-v8-4-implementation-evidence.md` |
