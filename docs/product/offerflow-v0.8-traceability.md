...
- **CLI / Windows 路径安全门（2026-08-05）：** Host Snapshot V3 四个 CLI 操作统一接入专用 `pathSafety`：只接受显式带盘符本地绝对路径，逐节点拒绝可识别 symlink/junction 并对既有输入使用 `realpath`；新输出要求真实父目录已存在，由 canonical parent + basename 构造且不再递归建父目录；拒绝 UNC/device namespace、点路径、尾随点/空格、保留设备名、SQLite sidecar、类型冒充、已存在输出、大小写/canonical 冲突和危险重叠。CLI 固定 32 参数 / 单参数 4096 / 总长度 16384 上限，控制字符、重复/缺失/多余/未知/非法布尔稳定拒绝且未知内容不回显。data/manifest 两个物理成员在打开前均须为 snapshot 内实际普通文件，读取使用文件描述符并在读前/读后复核 identity/size/time；组件数据当前内嵌于 data，无独立组件文件。边界仍明确：Node 无法可靠覆盖所有其他 Windows reparse tag，父链与发布不是 handle-relative，未宣称完全消除 TOCTOU；未修改 restore sidecar ownership、rename probe、report `.tmp`、清理失败语义或正式替换。
- **本切片验证：** 新增 CLI / pathSafety 定向 37/37、既有 Host Snapshot V3 30/30；TypeScript typecheck、production build、`git diff --check` 与定向临时文件残留检查通过。测试只使用系统临时目录与临时文件，未访问真实数据库或副本；当前 Windows 宿主不允许创建 file symlink（`EPERM`），因此 junction 使用真实 reparse point 验证，file symlink 分支使用 `lstat` 模拟覆盖且未静默 skip。
- **Restore 产物 ownership 与清理原子性（2026-08-06）：** `restore-candidate` 新增运行级内存 ledger；candidate 通过 `wx` 预占零长度文件并用 descriptor identity 登记，SQLite sidecar 在两次缺失预检、candidate ownership 与受控 SQLite 阶段的精确路径观察共同成立后才登记。固定 `<candidate>.rename-probe` 已删除，改为 128-bit 随机同目录 probe；报告临时文件使用随机名、`wx`、完整写入、`fsync`、close 与 identity 复核，最终报告通过同目录 hard-link no-replace 原子发布。清理只按 owned identity、依赖顺序和 3 次 `EPERM/EBUSY` 有限重试执行；主错误与清理错误同时保留稳定类别，已发布正式结果遇临时清理失败时保留 candidate/report 并拒绝报告完全成功。调用方预置 candidate/sidecar/report/旧固定 probe/未知文件的内容、hash 与时间保持不变；未扫描目录、未触碰真实库或副本。
- **进程中断矩阵与残留安全判定（2026-08-06）：** 新增 23 阶段 restore 磁盘边界模型，以及 13 个真实 child-process 强制终止点：candidate 预占、两套 schema bootstrap、两组件数据恢复、Host 校验前、Runtime validate 后、candidate 已移入 rename probe、report temp 创建/写入/fsync、final report 已发布但 temp 未删、candidate/report 已完整重验证但尚未返回。父进程通过确定性同步消息触发 Windows 强制终止并等待真实退出；逐点证明源库、snapshot data/manifest 和调用方预置文件 byte hash 不变、无 SQLite sidecar、未完成结果不冒充成功、第二个独立进程重跑不覆盖残留且返回稳定脱敏错误。测试 worker 仅接收系统临时根内的显式路径，禁用网络入口，不读取默认数据库路径或启动 HTTP。
- **只读 inspection 与成功重验证（2026-08-06）：** 新增 `NO_RESIDUE / CANDIDATE_WITHOUT_REPORT / REPORT_TEMP_WITHOUT_FINAL / FINAL_REPORT_WITH_TEMP_REMAINDER / CANDIDATE_AND_FINAL_REPORT_PRESENT / SQLITE_SIDECAR_PRESENT / AMBIGUOUS_OR_UNOWNED_RESIDUE` 七类判定，以及不含绝对路径、SQLite 原文或数据正文的人工处置计划。inspection 不删除、不 rename、不改写、不创建 sidecar、不推断 crash 后 ownership；身份不明文件仅建议“人工确认后处理”。candidate/report 只有在 report 普通文件与结构/Host digest 绑定、integrity/FK、OfferFlow/NovaWing component、Host 组合、正常 Runtime validate、无 sidecar/temp/probe和连接关闭全部重新通过后才标记 `verified`；篡改 candidate/report 或任一组件均拒绝。
- **Restore ownership 验证与边界（2026-08-06）：** ownership/cleanup 聚焦测试 25/25、Host Snapshot V3 37/37 已通过；本切片新增 interruption/phase 14/14 与 residue inspection 4/4，CLI/pathSafety、NovaWing Runtime Adapter、typecheck、production build 与残留检查结果见本轮交付记录。已覆盖指定关键阶段的进程级中断矩阵，但未实现通用崩溃恢复、自动残留删除、自动 rollback 或正式数据库替换；Node 父目录操作不是 handle-relative，随机 probe 预留删除到 candidate rename 之间仍存在已文档化的极小 TOCTOU，动态 sidecar 也没有操作系统级 creator attribution。本切片没有修改 Snapshot V3 数据/manifest 格式、表 registry、schema/migration、journal mode、feature flag、UI、Prompt、Provider、SSE、审批或正式记忆边界。
...
| 后台来源运行与定时发现 | 退役 | 2026-08-27 已从产品和运行环境移除 |
| 自动反馈画像进化 | 未纳入 | 不属于当前产品范围 |
| 低能量自动预筛 / 服务端批量 prefilter | 未纳入 | 不作为 V8-2 Done 条件 |
| 历史身份批量预检 / 自动滚动 /全自动巡航 | 退役 | 2026-08-27 已从产品和运行环境移除 |
