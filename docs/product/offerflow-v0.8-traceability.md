...
- **Restore ownership 验证与边界（2026-08-06）：** ownership/cleanup 聚焦测试 25/25、Host Snapshot V3 37/37 已通过；CLI/pathSafety、NovaWing Runtime Adapter、typecheck、production build 与残留检查结果见本轮交付记录。仍未实现完整进程崩溃矩阵或正式数据库替换；Node 父目录操作不是 handle-relative，随机 probe 预留删除到 candidate rename 之间仍存在已文档化的极小 TOCTOU，动态 sidecar 也没有操作系统级 creator attribution。本切片没有修改 Snapshot V3 数据/manifest 格式、表 registry、schema/migration、journal mode、feature flag、UI、Prompt、Provider、SSE、审批或正式记忆边界。
