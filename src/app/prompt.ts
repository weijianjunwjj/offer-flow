...
export function buildAnalysisPrompt(
  ...
  return [
    ...
    '第一部分是给人看的简洁 Markdown 简报（最多 5 段，每段不超过 3 行）；第二部分是给机器解析的 OFFER_FLOW_JSON 数据块。',
    '',
    '【求职者全局信息】',
    profileLines,
    '',
    ...
    '',
    '====================',
    '第一部分：Markdown 简报（给人看，最多 5 段，每段不超过 3 行）',
    '====================',
    '请先输出一行综合匹配度，格式：综合匹配度：XX%',
    '（XX 为 0-100 的单一整数百分比；不要写区间，难以精确时给一个代表值）',
    '随后用 Markdown 输出以下简洁结构：',
    '1. 岗位类型与技术栈匹配',
    '2. 项目经验匹配度',
    '3. 优势与风险',
    '4. 投递建议（四选一：强烈建议沟通 / 可以沟通 / 谨慎沟通 / 不建议浪费精力，并简述理由）',
    '5. Boss 打招呼话术（80 字以内，口语化）',
    '',
    '====================',
    '第二部分：OFFER_FLOW_JSON 数据块（给机器解析）',
    '====================',
    '在 Markdown 简报之后，另起一段，输出一个用固定标记包裹的 JSON 数据块，格式与示例完全一致：',
    '',
    ...
    '- opportunityAnalysis.bossGreeting：字符串（与第一部分第 5 点话术一致即可）',
    '',
    '【硬性要求，必须遵守】',
