...
* 启用生产 Radar / 执行新的生产 migration / 推送 main / Tag / Release 仍各自需要独立授权（本次 GA 仅更新文档口径，未 push / tag / release）；
...
```text
V8-2：CLOSED / FROZEN
V8-3：ACCEPTED / ACTIVATION PENDING（RC-05 / RC-06 = Done）
V8-4：持久化任务与可解释单岗位 AI 分析（已获开发授权，进行中）
```
...
* v0.8.0 已 GA（正式发布，2026-07-30，负责人批准）；本次 GA 仅更新文档口径，未 push / tag / release；
...
* 部分原 GA 前置项（RC-09、RC-12、30 条真实评测、核心页面截图与文案验收）未完成，经负责人明确豁免并转入 v0.9，不得标记为已完成；发布后阻断问题走 v0.8.x，普通优化与新能力进入 v0.9；
* 启用生产 Radar、真实 Provider 调用、执行新的生产 migration、推送 main / Tag / Release 仍各自需要独立授权。
