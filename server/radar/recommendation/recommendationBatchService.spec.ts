...
describe('RecommendationBatchService.createEmptyBatch（OPTION A 空批次领域契约）', () => {
  it('生成 0 条推荐的正式空批次：scope=[]、items=[]、emptyReason=no_candidates_in_scope', () => {
    const { db, service } = setup();
    const { batch, created } = service.createEmptyBatch();
    expect(created).toBe(true);
    expect(batch.status).toBe('succeeded');
    expect(batch.candidateVersionIds).toEqual([]);
    expect(batch.selectedCandidateVersionIds).toEqual([]);
    expect(batch.emptyReason).toBe('no_candidates_in_scope');
    // scope 冻结空请求集合 + 空收敛结果，是领域对象而非匿名 FK 占位符。
    const scope = batch.scope as {
      requestedCandidateVersionIds: string[];
      recommendationSet: { recommendations: unknown[]; emptyReason: string };
    };
    expect(scope.requestedCandidateVersionIds).toEqual([]);
    expect(scope.recommendationSet.recommendations).toEqual([]);
    expect(scope.recommendationSet.emptyReason).toBe('no_candidates_in_scope');
    // 真实落库、可回读，满足 daily_job_briefs.recommendation_batch_id 的 NOT NULL + FK。
    expect(service.getBatch(batch.id)?.id).toBe(batch.id);
    assertNoDownstreamWrites(db);
  });

  it('幂等：重复 createEmptyBatch 复用同一空批次（不插第二份）', () => {
    const { db, service } = setup();
    const first = service.createEmptyBatch();
    const second = service.createEmptyBatch();
    expect(first.created).toBe(true);
    expect(second.created).toBe(false);
    expect(second.batch.id).toBe(first.batch.id);
    const { count } = db.prepare('SELECT COUNT(*) count FROM radar_recommendation_batches').get() as { count: number };
  ...
  it('与 createBatch([]) 语义分离：createBatch 仍抛 SCOPE_EMPTY，createEmptyBatch 合法', () => {
    const { service } = setup();
    // 原契约不被 createEmptyBatch 意外改变。
    expect(() => service.createBatch([])).toThrow(/scope/);
    expect(() => service.createEmptyBatch()).not.toThrow();
  });
});
