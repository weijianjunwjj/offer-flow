...
export class StrategyStateVersionConflictError extends Error {
  constructor(readonly currentVersion: number) {
    super('求职策略状态版本冲突');
    this.name = 'StrategyStateVersionConflictError';
  }
}
...
/**
 * G5 求职策略正式持久化仓库（schema v6，仅限沙箱/临时库）。
 * 提案、正式版本、命令回执各自独立成表；单次命令在一个事务内以乐观并发
 * （state_version）读改写整份状态，与 G4 市场位置画像的持久化模式一致。
 */
export class StrategyRepository {
  ...
  private readMeta(): MetaRow | null {
    return (this.db
      .prepare('SELECT state_version, active_version_id FROM strategy_meta WHERE id = ?')
      .get(META_ID) as MetaRow | undefined) ?? null;
  }

  getState(): StrategyState {
    const meta = this.readMeta();
    if (meta === null) return createEmptyStrategyState();
    const proposals = (this.db
      .prepare('SELECT data_json FROM strategy_proposals ORDER BY created_at, id')
      .all() as Array<{ data_json: string }>)
      .map((row) => JSON.parse(row.data_json) as StrategyProposal);
    const versions = (this.db
      .prepare('SELECT data_json FROM strategy_versions ORDER BY version')
      .all() as Array<{ data_json: string }>)
      .map((row) => JSON.parse(row.data_json) as StrategyVersion);
    const commandReceipts = this.db
      .prepare(
        `SELECT idempotency_key AS idempotencyKey, command_type AS commandType,
         ...
         FROM strategy_receipts ORDER BY created_at, idempotency_key`,
      )
      .all() as StrategyState['commandReceipts'];
  ...
  private persist(state: StrategyState): void {
    this.db.prepare('DELETE FROM strategy_proposals').run();
    this.db.prepare('DELETE FROM strategy_versions').run();
    this.db.prepare('DELETE FROM strategy_receipts').run();
    ...
    for (const item of state.proposals) {
      insertProposal.run(
        ...
        JSON.stringify({ ...item, stale: false }), item.createdAt,
      );
    }
  ...
  updateState(
    ...
    update: (current: StrategyState) => StrategyState,
  ): StrategyState {
    const transaction = this.db.transaction(() => {
      const current = this.getState();
      if (current.stateVersion !== expectedStateVersion) {
        throw new StrategyStateVersionConflictError(current.stateVersion);
      }
      const next = StrategyStateSchema.parse(update(structuredClone(current))) as StrategyState;
      this.persist(next);
      return next;
    });
