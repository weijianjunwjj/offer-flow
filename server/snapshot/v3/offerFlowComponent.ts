...
export const OFFERFLOW_SCHEMA_V8_TABLE_REGISTRY: readonly OfferFlowTableRegistryEntry[] = [
  ...
  { name: 'import_logs', migrationVersion: 1, migrationName: V1, classification: 'temporary-audit', primaryModules: ['server/importLocalStorage', 'server/job-memory/upgrade', 'server/sync'], foreignKeys: [], includedInHostSnapshotV3: false, reason: '仅记录历史导入执行摘要，不是当前业务事实；正式结果已进入 profiles/jobs/Job Memory。V2 仍按旧契约保留该表。' },
  ...
  { name: 'radar_rule_assessments', migrationVersion: 7, migrationName: V7, classification: 'recomputable-derived', primaryModules: ['server/radar/ruleAssessmentRepository', 'server/radar/reviewService', 'server/radar/analysis/inputSnapshot'], foreignKeys: ['radar_candidates', 'radar_candidate_versions'], includedInHostSnapshotV3: false, reason: '由冻结 CandidateVersion 与规则版本确定性重算；分析任务已冻结实际使用的规则投影。' },
