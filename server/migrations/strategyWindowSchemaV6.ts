...
/**
 * G5 求职策略窗口与提案正式持久化（schema v6，仅限沙箱/临时库）。
 * 纯新增表，不修改 v2/v3/v4/v5 既有表与数据；提案、正式版本、命令回执各自独立成表，
 * 与 G4 市场位置画像的持久化模式一致。真实生产库（PRODUCTION_SCHEMA_VERSION 固定为 2）
 * 不得升级到 v6，也不在真实服务入口开启本能力。
 */
export const STRATEGY_WINDOW_SCHEMA_V6_SQL = `
...
CREATE TABLE strategy_receipts (
...
CREATE INDEX strategy_proposals_status_idx
  ON strategy_proposals(status, created_at);
CREATE INDEX strategy_versions_status_idx
  ON strategy_versions(status, version);
`;

export function createStrategyWindowSchemaV6(db: SqliteDatabase): void {
  db.exec(STRATEGY_WINDOW_SCHEMA_V6_SQL);
}
